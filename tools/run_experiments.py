...
def run_exp(cfg, configs):
    ...
    #                 '--config-file {} '.format(args.num_gpus, cfg)
    #     run_cmd(train_cmd)
...
def get_config(seed, shot):
    ...
    if args.coco:
        ...
        ITERS = {
            ...
            # 3: (800,1000,1200,1200,1200),
            # 5: (800,1200,2000,2000,2000),
            # 10: (1500,2100,2500,2500,2500),
            # 20: (1500,2100,3000,3000,3000),
            # 30: (1500,2100,3000,3000,3000),

            3: (800,1000,1201,1201,1201),
            5: (800,1200,2001,2001,2001),
            10: (1500,2100,2501,2501,2501),
            20: (1500,2100,3001,3001,3001),
            30: (1500,2100,3001,3001,3001),

            # 3: (400,800,800,800,800),
            # 5: (500,900,900,900,900),
            # 10: (500,900,900,900,900),
            # 20: (1500,2100,3000,3000,3000),
        }
    ...
    config = os.path.join(args.root, config_dir, temp + '.yml')
    ...
    configs['SOLVER']['MAX_ITER'] = ITERS[shot][4]
    configs['SOLVER']['STEPS'] = (ITERS[shot][0],ITERS[shot][1],ITERS[shot][2],ITERS[shot][3],)
    configs['SOLVER']['CHECKPOINT_PERIOD'] = ITERS[shot][3] 
    # // args.ckpt_freq
