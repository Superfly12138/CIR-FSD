...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.attention_loss=cfg.MODEL.ATTENTION.LOSS,
        self.attention_lossweight=cfg.MODEL.ATTENTION.LOSS_WEIGHT
    ...
    def forward(self, batched_inputs):
        ...
        if not self.training:
            features = self.backbone(images.tensor,None)
        else:
            features,c4_attention_loss = self.backbone(images.tensor,gt_instances)
        ...
        attention_loss=self.attention_loss[0]
        attention_lossweight=self.attention_lossweight
        c4_attention_losses = {"c4_attention_loss":  attention_lossweight * c4_attention_loss}
        losses = {}
        if attention_loss:
            losses.update(c4_attention_losses)
        losses.update(detector_losses)
        losses.update(proposal_losses)
        
        # print(c4_attention_losses)
        return losses
